...
def search(args):
    if not args:
        return []
    connection = sqlite3.connect('bars.db')
    connection.create_function("distance", 4, distance_between)
    c = connection.cursor()

    s = 'SELECT name, longitude, latitude, weighted_rank, distance(' + str(args[1]) + ', ' + str(args[0]) + ', longitude, latitude) AS walking_distance FROM bars WHERE walking_distance < ? GROUP BY name'

    bars = c.execute(s, (args[2],)).fetchall()
